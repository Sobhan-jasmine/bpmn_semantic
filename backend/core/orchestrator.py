...
The brain of the system. Owns the conversation loop and decides
which sub-module to invoke at each turn.
"""
...
class Orchestrator:
    ...
    async def create_process(
        ...
        json_output = await self.generator.generate(
            plan=plan,
            current_json={},
            context={}
        )
        ...
        return {
            ...
            "svg": None,  # Would fetch from SVG service
    ...
    async def revise_process(
        ...
        current_json = await self.memory_service.get_process_version(
            process_model_id, base_version_id
        )
        ...
        updated_json = await self.generator.generate(
            plan=plan,
        ...
        return {
            ...
            "version_id": version_id,
            "model_json": updated_json,
            "affected_elements": [],  # Would compute from diff
            "change_summary": f"Updated: {instruction}",
            "svg": None,
            "updated_at": datetime.now()
        }
    ...
    async def get_process(
        ...
        process_data = await self.memory_service.get_process(
            process_model_id
        )
    ...
    async def get_process_history(
        ...
        history = await self.memory_service.get_process_history(
            process_model_id
        )
